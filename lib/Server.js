...
class Server {
    ...
    request(cb) {
        this.app.on('request', async (req, res) => {
            ...
            serverEvent.emit('dataFinish', res, client);
        });
    }

    response(cb) {
        serverEvent.on('dataFinish', (res, client) => {
    ...
    async processHandler(client) {
        ...
        this.router.checkPreHandler(handler?.preHandler, client);
        client.data = method(body);
        this.router.checkPostHandler(handler?.postHandler, client);
        return client;
    }
