...
class Validator {
    static validate = (schema, body) => {
        if (!schema) return;
        if (!body) throw Error('empty request body');
        ...
        for (const key in schema) {
            ...
            if (income === undefined || income === null) {
                if (rule.require) throw Error(`required field: ${key}`);
            ...
            switch (rule.type) {
                ...
                case type.boolean:
                    ...
                    else throw Error(`invalid type: ${key}`);
    ...
    static checkReference = (filed, value, rule) => {
        ...
        for (const ref in currRef) {
            ...
            if (isFunc) {
                if (!curr(value, currRef[ref])) throw Error(`invalid value: ${filed}`);
            } else {
                for (const key in currRef[ref]) {
                    const opt = curr[key];
                    const optVal = currRef[ref][key];
                    if (!opt(value, optVal)) throw Error(`invalid value: ${filed}`);
                }
            }
        }
    }
